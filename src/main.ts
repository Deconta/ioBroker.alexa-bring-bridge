...
class AlexaBringBridge extends utils.Adapter {
            ...
            try {
                ...
                if (itemFound && itemNameRaw) {
                    ...
                    itemsToProcess.forEach(item => {
                        let matchedRecipe = this.config.recipesList?.find(r => r.name.toLowerCase() === item.trim().toLowerCase());
                    ...
                    itemsToProcess.forEach(async (singleItemRaw, index) => {
                        ...
                        // 1. Blacklist-Check
                        const isBlacklisted = this.config.blacklist?.some(b => b.word.toLowerCase() === itemToProcess.toLowerCase());
                        if (isBlacklisted) {
                        ...
                        // 2. Synonym-Check
                        const synonymMatch = this.config.synonymsList?.find(s => s.original.toLowerCase() === itemToProcess.toLowerCase());
                        ...
                        if (cleanedItemName) {
                            ...
                            try {
                                ...
                                if (bringContentState && bringContentState.val) {
                                    ...
                                    let existingItem = bringList.find((i: any) => i.name.toLowerCase() === cleanResult.product.toLowerCase());
    ...
    private stripPrefixes(text: string): string {
        ...
        const prefixesToRemove = [
            /^(?:hey\s+)?alexa,\s*/i,
            /^(?:hey\s+)?alexa\s+/i,
            /^füge\s+/i, /^setz\s+/i, /^setze\s+/i, /^packe\s+/i, /^pack\s+/i, 
            /^schreib\s+/i, /^schreibe\s+/i, /^notiere\s+/i
        ];
    ...
    private wordsToNumbersSmart(text: string): string {
        ...
        try {
            ...
            const ones: Record<string, number> = {
                "null": 0, "eins": 1, "eine": 1, "einen": 1, "ein": 1, "milli": 1,
    ...
    private async removeItemFromAlexaList(itemName: string, extractedProduct: string, attempt: number = 1): Promise<void> {
        const maxAttempts = 5; 
